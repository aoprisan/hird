...
| [`recess.sh`](recess.sh) | **The human stands the queue down** — `hird recess` refuses new claims in the human's words, tells `task_next` callers to stand by rather than "the queue is empty", keeps the dispatch hook quiet, and `hird resume` announces everything that waited as `resumed`. |
...
| [`harness/`](harness) | MCP registration for Claude Code, Codex CLI, Copilot in VS Code, the Copilot CLI, the Gemini CLI and OpenCode — what `hird register <harness>` writes, for reading or for pasting somewhere it does not reach. |
